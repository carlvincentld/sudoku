...
export class JigsawGrid extends Grid {
	...
	constructor(
		...
		if (!skipShuffle) this.shuffleSections();
	}

	override clone(): JigsawGrid {
		const result = new JigsawGrid(
			this.WIDTH,
			this.HEIGHT,
			this.SECTION_COUNT,
			this.random,
	...
	private swap(): boolean {
		...
		for (const section of sections) {
			...
			for (const neighbour of shuffle(neighbouringCells, this.random)) {
				const cellsNextToNeighbour = cellsByNeighbouringSection.get(
					neighbour.section
				)!;
				for (const cellNextToNeighbour of shuffle(
					cellsNextToNeighbour,
					this.random
				)) {
	...
	private neighbouringCells(cell: Cell): Cell[] {
		...
		return neighboursPosition.reduce((acc, x) => {
	...
	private isSectionContiguous(section: Section): boolean {
		...
		while (toVisit.length) {
			...
			toVisit.push(
				...this.neighbouringCells(cell).filter((x) => x.section === section)
			);
		}
